...
#Concatenate foaf type descriptions accoringly. 
collapsByFoafType <- function(ontologyText, breakText){
  newText <- c()
  n <- 1
...
foafTypeText.v <- collapsByFoafType(foafText.v, "[#]  [back to top]")
length(foafTypeText.v)
foafTypeText.v[1]

foafTypeDescription.m <- cbind(foafTypes.v ,foafTypeText.v)
...
corpus.m <- cbind(foafTypes.v, foafTypeText.v)
corpus.df <- as.data.frame(corpus.m, stringsAsFactors = FALSE)
corpus.df <- within(corpus.df, conc <- paste(corpus.m[,1],corpus.m[,2], sep = " "))
